...
export class SelectAttributeComponent implements ControlValueAccessor, OnChanges {
  ...
  nodeSelected(field: FieldNode, node: DynamicEntityNode, pathOnly?: boolean) {
    ...
    this.onChange(selectedFieldNode);
    this.onTouched();
  ...
  get isValidNode(): boolean {
    return !!this.selectedNode?.name && !!this.selectedNode?.type;
  }

  get breadcrumbs(): string[] {
    return this.selectedNode?.name?.split('.') ?? [];
  }
  ...
  takeFieldName(field: FieldNode, node: DynamicEntityNode) : string {
    let parentPath = '';
    node.path.forEach((nodeName: string) => {
  ...
  private onChange = (_: FieldNode | null) => {};
  private onTouched = () => {};
  ...
  registerOnChange(fn: (_: any) => void): void { /* console.log('On Change method updated');*/ this.onChange = fn; }
  registerOnTouched(fn: any): void { /* console.log('On Touched method updated'); */ this.onTouched = fn; }
  setDisabledState(isDisabled: boolean): void {this.isDisabled = isDisabled;}

  // Writes a new value from the form model into the view or (if needed) DOM property.
  writeValue(value: FieldNode | null) {
